...
// const FlightTimelineInfo = ({booking}) => {
//   // booking이 없거나 flight 타입이 없으면 null
//   if (!booking || !booking.types?.includes('flight') || !booking.productIds?.length) {
//     return null;
//   }

//   // flight만 필터링
//   const flights = booking.productIds.filter(item => item.type === 'flight');
//   if (!flights.length) return null;

//   // 각 항공편을 렌더링하는 함수
//   const renderFlightCard = (flightData, idx) => {
//     // 날짜/요일
//     const rawDate = flightData.departure?.date;
//     const formattedDate = rawDate ? dayjs(rawDate).format('YYYY-MM-DD') : null;
//     const monthDay = formattedDate
//       ? `${parseInt(formattedDate.split('-')[1])}월 ${parseInt(
//           formattedDate.split('-')[2]
...
//     return (
//       <Box sx={{position: 'relative', mb: 4}} key={idx}>
//         {/* 왼쪽에 날짜/요일 타임라인 */}
//         <Box sx={{position: 'relative', pl: 3, mb: 2}}>
//           {/* 둥근 점 */}
//           <Box
...
//             <Box sx={{mx: 1}}>
//               <FlightTakeoffIcon sx={{color: 'text.secondary', mr: 0.5}} />
//               {/* 중간 구분선 대신 다른 아이콘/텍스트를 원하면 여기에 배치 */}
//             </Box>
...
//     <Box>
//       {/* 여러 항공편을 순회하며 각각 렌더링 */}
//       {flights.map((flightData, index) => renderFlightCard(flightData, index))}
//     </Box>
//   );
// };
